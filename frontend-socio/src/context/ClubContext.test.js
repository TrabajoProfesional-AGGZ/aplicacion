...
import { clubEnMemoria } from '../services/clubService';
...
describe('ClubProvider', () => {
  ...
  test('arranca con el club cacheado, para que un boot offline ya tenga la marca blanca', async () => {
    clubEnMemoria.mockReturnValue(CLUB);
    // Sin red el lookup falla, y el provider se queda con lo cacheado en vez de borrarlo.
    resolverClub.mockResolvedValue(CLUB);
    await montar();

    expect(screen.getByText('club: Club Uno')).toBeInTheDocument();
    clubEnMemoria.mockReturnValue(null);
  });
